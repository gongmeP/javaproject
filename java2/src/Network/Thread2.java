...
//			System.out.println(result2);
		...
		while(a < user1.length) {
			...
			th2 = new Thread(lp2); //Thread 이용하여 해당 데이터 처리
			th3 = new Thread(lp3);
//			th2.start(); //각각의 Thread 가 작동하도록 함
//			th3.start();
			a++;
		}
		...
		try {
			...
			while(k < user1.length) {
				...
				tt1.start(); //멀티쓰레드로 작업 시작 
//				tt1.join(); //멀티로 작업 후 순차적으로 결과값이 출력되도록 조정함 Thread.sleep(100); 딜레이줌
//				tt2.start();
//				tt2.join();
//				tt3.start();
//				tt4.start();
